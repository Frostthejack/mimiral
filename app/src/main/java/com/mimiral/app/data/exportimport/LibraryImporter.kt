    ...
    suspend fun importLibrary(json: String): ImportResult = withContext(Dispatchers.IO) {
        ...
        for (exportProgress in export.readingProgress) {
            try {
                ...
                errors.add("Failed to import reading progress for book ${exportProgress.bookId}: ${e.message}")
            }
        }
        ...
        for (exportBookmark in export.bookmarks) {
            try {
                ...
                errors.add("Failed to import bookmark for book ${exportBookmark.bookId}: ${e.message}")
            }
        }
        ...
        for (exportHighlight in export.highlights) {
            try {
                ...
                errors.add("Failed to import highlight for book ${exportHighlight.bookId}: ${e.message}")
            }
        }
        ...
        for (exportBookCol in export.bookCollections) {
            try {
                ...
                val newColId = collectionIdMap[exportBookCol.collectionId] ?: exportBookCol.collectionId
